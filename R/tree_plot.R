        ...
        if (k$var[i] %in% k$father_name){
            for (j in 1:length(k$leafnode_value[[which(k$var[i] == k$father_name)]])){
                ...
                if (!identical(k$leaf_name[[which(k$var[i] == k$father_name)]],NA)){
                    text(xlim-55,20*len-7,k$leaf_name[[which(k$var[i] == k$father_name)]][j])
                    if (!identical(k$prob[[i]],NA)) text(xlim-55,20*len,k$prob[[i]][j])
                }
                else if (!identical(k$prob[[i]],NA)) text(xlim-55,20*len-7,k$prob[[i]][j])
                coor$y[j] <- 20*len-10
                len <- len-1
            }
            for (j in 1:length(k$leafnode_value[[which(k$var[i] == k$father_name)]])){
            ...
            for (name in k$sub_name[[i]]){
                if (name %in% k$var){
                    ...
                    text(x_one(xlim,md,k$yval[i])+20,coor[[name]]+3,name)
                    ...
                    text(x_one(xlim,md,k$yval[i])+20,20*len-7,name)
